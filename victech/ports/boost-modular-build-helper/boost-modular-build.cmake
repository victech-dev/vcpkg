function(boost_modular_build)
    ...
    elseif(VCPKG_CMAKE_SYSTEM_NAME STREQUAL "Linux" AND VCPKG_TARGET_ARCHITECTURE STREQUAL "arm")
    ...
    if(NOT EXISTS "${BOOST_BUILD_PATH}")
        if(VCPKG_CMAKE_SYSTEM_NAME STREQUAL "Linux" AND VCPKG_TARGET_ARCHITECTURE STREQUAL "arm64")
            if(CMAKE_HOST_SYSTEM_PROCESSOR STREQUAL "aarch64")
                message(FATAL_ERROR "${CMAKE_HOST_SYSTEM_PROCESSOR}) The arm64 boost-build tools must be installed to build arm64 for Linux. Please run `vcpkg install boost-build:arm64-linux`.")
            else()
                message(FATAL_ERROR "${CMAKE_HOST_SYSTEM_PROCESSOR}) The x64 boost-build tools must be installed to build arm64 for Linux. Please run `vcpkg install boost-build:x64-linux`.")
            endif()
        elseif(VCPKG_CMAKE_SYSTE_NAME STREQUAL "Linux" AND VCPKG_TARGET_ARCHITECTURE STREQUAL "arm")
            if(CMAKE_HOST_SYSTEM_PROCESSOR STREQUAL "armv7l")
                message(FATAL_ERROR "${CMAKE_HOST_SYSTEM_PROCESSOR}) The arm boost-build tools must be installed to build arm64 for Linux. Please run `vcpkg install boost-build:arm-linux`.")
            else()
                message(FATAL_ERROR "${CMAKE_HOST_SYSTEM_PROCESSOR}) The x64 boost-build tools must be installed to build arm64 for Linux. Please run `vcpkg install boost-build:x64-linux`.")
            endif()
        else()
